...
insertTxOuts trce blkNo (ShelleyTx.TxIn txInId _, txOut) = do
  ...
  _ <- insertStakeAddressRefIfMissing trce blkNo (txOutAddress txOut)
  void . DB.insertTxOut $
            DB.TxOut
              { DB.txOutTxId = txId
              , DB.txOutIndex = 0
...
insertStaking
    :: (MonadBaseControl IO m, MonadIO m)
    => Trace IO Text -> BlockNo -> ShelleyGenesis StandardShelley
    -> ExceptT SyncNodeError (ReaderT SqlBackend m) ()
insertStaking tracer blkNo genesis = do
  ...
  forM_ params $ uncurry (insertPoolRegister tracer (Left 2) (sgNetworkId genesis) 0 blkNo txId)
  let stakes = zip [0..] $ Map.toList (sgsStake $ sgStaking genesis)
  forM_ stakes $ \(n, (keyStaking, keyPool)) ->
    insertDelegation tracer (sgNetworkId genesis) 0 0 blkNo n Nothing (KeyHashObj keyStaking) [] keyPool
