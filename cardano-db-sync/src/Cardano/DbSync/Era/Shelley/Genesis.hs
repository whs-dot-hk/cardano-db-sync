            ...
            when (hasInitialFunds || hasStakes) $ do
                ...
                void . lift . DB.insertBlock $
                          DB.Block
                            ...
                            , DB.blockBlockNo = unBlockNo DB.shelleyGenesisBlockNo
...
insertStaking tracer cache blkNo genesis = do
  ...
  forM_ params $ uncurry (insertPoolRegister tracer cache (Left 2) (sgNetworkId genesis) 0 blkNo txId)
  let stakes = zip [0..] $ Map.toList (sgsStake $ sgStaking genesis)
  forM_ stakes $ \(n, (keyStaking, keyPool)) -> do
    insertStakeRegistration (EpochNo 0) blkNo txId (2 * n) (Generic.annotateStakingCred (sgNetworkId genesis) (KeyHashObj keyStaking))
    insertDelegation cache (sgNetworkId genesis) 0 0 blkNo n Nothing (KeyHashObj keyStaking) [] keyPool
