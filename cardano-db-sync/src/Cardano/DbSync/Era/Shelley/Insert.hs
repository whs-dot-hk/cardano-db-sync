...
insertMaTxOuts
    :: (MonadBaseControl IO m, MonadIO m)
    => Trace IO Text -> Cache -> DB.TxOutId -> Map (PolicyID StandardCrypto) (Map AssetName Integer)
    -> ExceptT SyncNodeError (ReaderT SqlBackend m) [DB.MaTxOutId]
insertMaTxOuts _tracer cache txOutId maMap =
    concatMapM (lift . insertOuter) $ Map.toList maMap
  where
    insertOuter
        :: (MonadBaseControl IO m, MonadIO m)
        => (PolicyID StandardCrypto, Map AssetName Integer)
        -> ReaderT SqlBackend m [DB.MaTxOutId]
    insertOuter (policy, aMap) =
