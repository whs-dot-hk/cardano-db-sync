...
assertStakeAddressCount :: DBSyncEnv -> Word -> IO ()
assertStakeAddressCount env n =
    assertEqBackoff env qry n defaultDelays "Unexpected block count"
  where
    qry = maybe 0 unValue . listToMaybe <$>
            (select . from $ \(_a :: SqlExpr (Entity StakeAddress)) -> pure countRows)
...
assertAddrValues
    :: (Crypto era ~ StandardCrypto, HasField "address" (Core.TxOut era) (Ledger.Addr (Crypto era)))
    => DBSyncEnv -> UTxOIndex era -> DbLovelace -> LedgerState (ShelleyBlock era)
    -> IO ()
