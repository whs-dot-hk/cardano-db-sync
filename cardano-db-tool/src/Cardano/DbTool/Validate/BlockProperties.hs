...
validateBlockTimesInPast = do
  ...
  where
    reportFailures :: [(Maybe Word64, Word64, UTCTime)] -> String
    reportFailures xs =
      mconcat
    ...
    showFirst :: (Maybe Word64, Word64, UTCTime) -> String
...
queryBlockNoList start count = do
  res <- select $ do
    ...
    where_ (blk ^. BlockBlockNo >. val start)
...
queryBlocksTimeAfters :: MonadIO m => UTCTime -> ReaderT SqlBackend m [(Maybe Word64, Word64, UTCTime)]
