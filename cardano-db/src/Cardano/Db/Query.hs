...
queryBlockHash hash = do
  ...
  pure $ maybeToEither (DbLookupBlockHash hash) BlockNo (fmap unValue $ listToMaybe res)
...
queryBlockHeight = do
  res <- select $ do
            ...
            where_ (blk ^. BlockBlockNo <. val (unBlockNo shelleyGenesisBlockNo))
...
queryByronGenesisSupply = do
  res <- select $ do
          ...
          where_ (tx ^. TxBlockNo ==. val (unBlockNo byronGenesisBlockNo))
          pure $ sum_ (txOut ^. TxOutValue)
  pure $ unValueSumAda (listToMaybe res)
...
queryByronGenesisTxCount = do
  res <- select $ do
          blk <- from $ table @Block
          where_ (blk ^. BlockBlockNo ==. val (unBlockNo byronGenesisBlockNo))
...
queryDepositUpToBlockNo blkNo = do
  res <- select $ do
    (tx :& blk) <-
      ...
      `innerJoin` table @Block
      `on` (\(tx :& blk) -> tx ^. TxBlockNo ==. blk ^. BlockBlockNo)
    where_ (blk ^. BlockBlockNo <=. val blkNo)
...
queryShelleyGenesisSupply = do
  res <- select $ do
          ...
          where_ (tx ^. TxBlockNo ==. val (unBlockNo shelleyGenesisBlockNo))
          pure $ sum_ (txOut ^. TxOutValue)
  pure $ unValueSumAda (listToMaybe res)
...
txLessEqual blkid =
  ...
  where
    ...
    blockLessEqual :: SqlExpr (ValueList Word64)
